...
import { createCombinedTestData } from './cubitGeneratorNew';

// Интерфейсы для webview (дублируем только необходимые)
interface StateInfo {
    name: string;
    properties?: string[];
    isInitial?: boolean;
 ...
 */
export function createTestAnalysisWebview(
    cubitName: string,
...
function generateHTMLContent(
    ...
    totalTestPaths: number,
    methodsInfo: string,
    branchesInfo: string,
    combinedPathsInfo: string,
    fsmDot: string,
    pathsDot: string
): string {
    return `<!DOCTYPE html>
 ...
 */
function getWebviewJavaScript(fsmDot: string, pathsDot: string): string {
    return `
