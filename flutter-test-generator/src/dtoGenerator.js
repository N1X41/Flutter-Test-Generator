...
async function generateTest(workspaceRoot, testDir, featureName, dtoName, dtoCode) {
    const testUri = vscode.Uri.joinPath(workspaceRoot, testDir, 'features', featureName, 'dto', `${dtoName}_test.dart`);
    const dirUri = vscode.Uri.joinPath(workspaceRoot, testDir, 'features', featureName, 'dto');
    await vscode.workspace.fs.createDirectory(dirUri);
    const content = new util_1.TextEncoder().encode(dtoCode);
