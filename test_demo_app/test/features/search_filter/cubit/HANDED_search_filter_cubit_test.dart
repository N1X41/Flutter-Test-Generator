...
void main() {
  ...
  group('Тесты для SearchFilterCubit', () {
    ...
    blocTest<SearchFilterCubit, SearchFilterState>(
      ...
      act: (cubit) => cubit.applyFilters({'': 1}),
      expect: () => [const SearchFilterAppliedState(filters: {'': 1}, activeFiltersCount: 1)],
    );
    blocTest<SearchFilterCubit, SearchFilterState>(
      ...
      expect: () =>
          [const SearchFilterLoadingState(), const SearchFilterAppliedState(filters: {}, activeFiltersCount: 0)],
      verify: (_) {
    ...
    blocTest<SearchFilterCubit, SearchFilterState>(
      ...
      act: (cubit) async {
        cubit.resetToInitial();
        cubit.applyFilters({'': 1});
      },
      expect: () => [SearchFilterInitialState(), SearchFilterAppliedState(filters: {'': 1}, activeFiltersCount: 1)],
    );
    blocTest<SearchFilterCubit, SearchFilterState>(
      ...
      expect: () => [
        SearchFilterInitialState(),
        SearchFilterLoadingState(),
        SearchFilterAppliedState(filters: {}, activeFiltersCount: 0)
      ],
      verify: (_) {
        verify(() => mockISecureStorageService.read(any())).called(1);
      },
